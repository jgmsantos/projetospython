...
def plot_legend(axes):
  ...
  #GM mudei de fontsize=6 para fontsize=7. Mudei de loc=(0.125,0) para loc=(0.1,0.3)
  plt.legend((lT, lTve, lTd, lTwb, lPT, lTvp,),(tT, tTve, tTd, tTwb, tPT, tTvp,),
             loc=(0.09,0.3), fontsize=6.5, handlelength=10)
...
def plot_datablock(ax4, _x,_z,_t,_th,_p,_qv,_u,_v, _title):
  ...
  ax4.set_axis_off()
  plt.axis([-1,1,-1,1])
  #GM mudei de fontsize=10 para fontsize=7
  plt.text(0,1,_title, verticalalignment='top', horizontalalignment='center', weight='bold', fontsize=8)
  line = ""
  if (_x != None):
  ...
  plt.text(0,.85, line, verticalalignment='center', horizontalalignment='center', fontsize=7)  #GM mudei de 5 para 7.
  ...
  plt.text(-1,.75, line, verticalalignment='center', horizontalalignment='left', fontsize=7)  #GM mudei de 5 para 7.
  ...
  x = 0.4
  y = 0
  # Mudei de 5 para 7 no fontsize. #GM
  line = 'Hodograph'
  plt.text(x,y, line, verticalalignment='center', horizontalalignment='left', fontsize=7)
  x += 0.02
  y -= 0.065
  line = '0-1 km shear {0:3d}$^\circ$ {1:3.1f} m/s'.format(int(shear['s01'][0]),shear['s01'][1])
  plt.text(x,y, line, verticalalignment='center', horizontalalignment='left', fontsize=7)
  # Mudei de y -= 0.05 para y -= 0.08 #GM
  y -= 0.08
...
def print_3col(name, value, unit, x, y):
   # mudei de fontsize=5 para fontsize=7
   plt.text(x,y, name, verticalalignment='center', horizontalalignment='left', fontsize=7)  #GM
   plt.text(x+.35,y, value, verticalalignment='center', horizontalalignment='right', fontsize=7)  #GM
   plt.text(x+.4,y, unit, verticalalignment='center', horizontalalignment='left', fontsize=7)  #GM

def print_parcel_info(title, pcl, x, y):
  # mudei de fontsize=5 para fontsize=7  
  plt.text(x,y, title, verticalalignment='center', horizontalalignment='left', fontsize=7)  #GM
  y -= 0.065
  x += 0.02
  print_3col('CAPE', '{0}'.format(int(pcl['cape'])), 'J kg$^{-1}$', x, y)
  # Mudei de y -= 0.05 para y -= 0.071 #GM
  y -= 0.071
...
def label(x, y, s, c, r, axes):
  # Tamanho da fonte dos valores dos níveis verticais do skewt.
	axes.text(x,y*100,s, verticalalignment='center', horizontalalignment='center', weight='bold', fontsize=6*fontscalefactor, color=c, rotation=r)

# Tamanho da fonte dos eixos x e y do hodógrafo.
def label_h(x, y, s, c, r, axes):
	axes.text(x,y,s, verticalalignment='center', horizontalalignment='center', weight='normal', fontsize=8*fontscalefactor, color=c, rotation=r)

# Tamanho da fonte dos valores do hodógrafo.
...
# Tamanho da fonte dos rótulos que aparecem no canto infeior direito do skewt.
def label_m(x, y, s, axes):
	axes.text(x,y,s, verticalalignment='center', horizontalalignment='right', fontsize=7*fontscalefactor)
